...
import type {VipPackageInput} from '@/apis/__generated/model/static'
...
const initForm: VipPackageInput = {}
const {formData: form, restForm} = useFormHelper<VipPackageInput>(initForm)
const rules = reactive<FormRules<VipPackageInput>>({
...
const handleConfirm = () => {
  formRef.value?.validate(
      assertFormValidate(() =>
          api.vipPackageForAdminController.save({body: form.value}).then(async (res) => {
            form.value.id = res
...
onActivated(() => {
  if (props.id) {
    api.vipPackageForAdminController.findById({id: props.id}).then((res) => {
      form.value = res
    })
  } else {
    restForm()
