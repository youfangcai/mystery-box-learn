...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="盲盒名字" prop="name" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:MysteryBoxScope">
          <dict-column :dict-id="DictConstants." :value="row.name"></dict-column>
        </template>
      </el-table-column>
      <el-table-column label="盲盒详情" prop="details" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:MysteryBoxScope">
          <dict-column :dict-id="DictConstants." :value="row.details"></dict-column>
        </template>
      </el-table-column>
      <el-table-column label="购买提示" prop="tips" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:MysteryBoxScope">
          <dict-column :dict-id="DictConstants." :value="row.tips"></dict-column>
        </template>
      </el-table-column>
      <el-table-column label="价格" prop="price" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:MysteryBoxScope">
          <dict-column :dict-id="DictConstants." :value="row.price"></dict-column>
        </template>
      </el-table-column>
      <el-table-column label="封面" prop="cover" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:MysteryBoxScope">
          <dict-column :dict-id="DictConstants." :value="row.cover"></dict-column>
        </template>
      </el-table-column>
