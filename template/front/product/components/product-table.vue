...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="名称" prop="name" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:ProductScope">
          {{row.name}}
        </template>
      </el-table-column>
      <el-table-column label="价格" prop="price" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:ProductScope">
          {{row.price}}
        </template>
      </el-table-column>
      <el-table-column label="封面" prop="cover" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:ProductScope">
          <el-avatar :src="row.cover" alt=""></el-avatar>
        </template>
      </el-table-column>
      <el-table-column label="品牌" prop="brand" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:ProductScope">
          {{row.brand}}
        </template>
      </el-table-column>
      <el-table-column label="类别" prop="categoryId" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:ProductScope">
          {{row.category.name}}
        </template>
      </el-table-column>
      <el-table-column label="描述" prop="description" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:ProductScope">
          {{row.description}}
        </template>
      </el-table-column>
      <el-table-column label="标签" prop="tags" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:ProductScope">
          {{row.tags}}
        </template>
      </el-table-column>
      <el-table-column label="规格" prop="specifications" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:ProductScope">
          {{row.specifications}}
        </template>
      </el-table-column>
      <el-table-column label="属性" prop="attributes" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:ProductScope">
          {{row.attributes}}
        </template>
      </el-table-column>
