...
import type {RefundRecordDto} from '@/apis/__generated/model/dto'
...
type RefundRecordScope = Scope<RefundRecordDto['RefundRecordRepository/COMPLEX_FETCHER_FOR_ADMIN']>
const refundRecordTableHelper = inject(
    'refundRecordTableHelper',
    useTableHelper(api.refundRecordForAdminController.query, api.refundRecordForAdminController, {})
)
const {
  loadTableData,
  handleSortChange,
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.refundRecordForAdminController.delete({body: ids}).then((res) => {
  ...
  <div>
    ...
    >
      ...
      <el-table-column label="创建时间" prop="createdTime" show-overflow-tooltip sortable="custom" width="150">
        <template v-slot:default="{ row }: RefundRecordScope">
          {{ row.createdTime }}
        </template>
      </el-table-column>
