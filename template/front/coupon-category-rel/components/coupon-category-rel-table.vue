...
import type {CouponCategoryRelDto} from '@/apis/__generated/model/dto'
...
type CouponCategoryRelScope = Scope<CouponCategoryRelDto['CouponCategoryRelRepository/COMPLEX_FETCHER_FOR_ADMIN']>
const couponCategoryRelTableHelper = inject(
    'couponCategoryRelTableHelper',
    useTableHelper(api.couponCategoryRelForAdminController.query, api.couponCategoryRelForAdminController, {})
)
const {
  loadTableData,
  handleSortChange,
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.couponCategoryRelForAdminController.delete({body: ids}).then((res) => {
...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="商品分类" prop="categoryId" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CouponCategoryRelScope">
          {{row.category.name}}
        </template>
      </el-table-column>
