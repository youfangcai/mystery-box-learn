...
const {formData: form, restForm} = useFormHelper<DictInput>(initForm)
const rules = reactive<FormRules<DictInput>>({})
const handleConfirm = () => {
  formRef.value?.validate(
      assertFormValidate(() =>
          api.dictForAdminController.save({body: form.value}).then(async (res) => {
            form.value.id = res
...
onActivated(() => {
  if (props.id) {
    api.dictForAdminController.findById({id: props.id}).then((res) => {
      form.value = res
    })
  } else {
    restForm()
