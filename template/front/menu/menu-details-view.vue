...
const {formData: form, restForm} = useFormHelper<MenuInput>(initForm)
const rules = reactive<FormRules<MenuInput>>({})
const handleConfirm = () => {
  formRef.value?.validate(
      assertFormValidate(() =>
          api.menuForAdminController.save({body: form.value}).then(async (res) => {
            form.value.id = res
...
onActivated(() => {
  if (props.id) {
    api.menuForAdminController.findById({id: props.id}).then((res) => {
      form.value = res
    })
  } else {
    restForm()
