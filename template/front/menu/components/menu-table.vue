...
type MenuScope = Scope<MenuDto['MenuRepository/COMPLEX_FETCHER_FOR_ADMIN']>
const menuTableHelper = inject(
    'menuTableHelper',
    useTableHelper(api.menuForAdminController.query, api.menuForAdminController, {})
)
const {
  loadTableData,
  handleSortChange,
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.menuForAdminController.delete({body: ids}).then((res) => {
