...
import type {UserWeChatDto} from '@/apis/__generated/model/dto'
...
type UserWeChatScope = Scope<UserWeChatDto['UserWeChatRepository/COMPLEX_FETCHER_FOR_ADMIN']>
const userWeChatTableHelper = inject(
    'userWeChatTableHelper',
    useTableHelper(api.userWeChatForAdminController.query, api.userWeChatForAdminController, {})
)
const {
  loadTableData,
  handleSortChange,
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.userWeChatForAdminController.delete({body: ids}).then((res) => {
  ...
  <div>
    ...
    >
      ...
      <el-table-column label="创建时间" prop="createdTime" show-overflow-tooltip sortable="custom" width="150">
        <template v-slot:default="{ row }: UserWeChatScope">
          {{ row.createdTime }}
        </template>
      </el-table-column>
