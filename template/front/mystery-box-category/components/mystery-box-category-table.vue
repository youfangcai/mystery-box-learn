...
type MysteryBoxCategoryScope = Scope<MysteryBoxCategoryDto['MysteryBoxCategoryRepository/COMPLEX_FETCHER_FOR_ADMIN']>
const mysteryBoxCategoryTableHelper = inject(
    'mysteryBoxCategoryTableHelper',
    useTableHelper(api.mysteryBoxCategoryForAdminController.query, api.mysteryBoxCategoryForAdminController, {})
)
const {
  loadTableData,
  handleSortChange,
...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="类别名称" prop="name" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:MysteryBoxCategoryScope">
          {{row.name}}
        </template>
      </el-table-column>
      <el-table-column label="类别图标" prop="icon" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:MysteryBoxCategoryScope">
      ...
      <el-table-column label="排序号" prop="sortOrder" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:MysteryBoxCategoryScope">
          {{row.sortOrder}}
        </template>
      </el-table-column>
