...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="过期时间" prop="endTime" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:VipScope">
          {{row.endTime}}
        </template>
      </el-table-column>
      <el-table-column label="用户信息" prop="user" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:VipScope">
          {{row.user.name}}
        </template>
      </el-table-column>
