...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="优惠券名称" prop="name" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CouponScope">
          {{row.name}}
        </template>
      </el-table-column>
      <el-table-column label="使用门槛" prop="thresholdAmount" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CouponScope">
          {{row.thresholdAmount}}
        </template>
      </el-table-column>
      <el-table-column label="发放数量" prop="releasedQuantity" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CouponScope">
          {{row.releasedQuantity}}
        </template>
      </el-table-column>
      <el-table-column label="生效日期" prop="effectiveDate" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CouponScope">
          {{row.effectiveDate}}
        </template>
      </el-table-column>
      <el-table-column label="过期时间" prop="expirationDate" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CouponScope">
          {{row.expirationDate}}
        </template>
      </el-table-column>
      <el-table-column label="优惠类型" prop="couponType" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CouponScope">
          <dict-column :dict-id="DictConstants.COUPON_TYPE" :value="row.couponType"></dict-column>
        </template>
      </el-table-column>
      <el-table-column label="使用范围" prop="scopeType" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CouponScope">
          <dict-column :dict-id="DictConstants.COUPON_SCOPE_TYPE" :value="row.scopeType"></dict-column>
        </template>
      </el-table-column>
      <el-table-column label="优惠金额" prop="amount" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CouponScope">
          {{row.amount}}
        </template>
      </el-table-column>
      <el-table-column label="折扣" prop="discount" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CouponScope">
          {{row.discount}}
        </template>
      </el-table-column>
      <el-table-column label="是否启用" prop="status" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CouponScope">
