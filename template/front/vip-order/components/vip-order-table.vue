...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.vipOrderForAdminController.delete({body: ids}).then((res) => {
...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="用户" prop="user" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:VipOrderScope">
          {{row.user.name}}
        </template>
      </el-table-column>
      <el-table-column label="vip套餐" prop="vipPackage" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:VipOrderScope">
          {{row.vipPackage.name}}
        </template>
      </el-table-column>
