...
import type {MysteryBoxProductRelDto} from '@/apis/__generated/model/dto'
...
type MysteryBoxProductRelScope = Scope<MysteryBoxProductRelDto['MysteryBoxProductRelRepository/COMPLEX_FETCHER_FOR_ADMIN']>
const mysteryBoxProductRelTableHelper = inject(
    'mysteryBoxProductRelTableHelper',
    useTableHelper(api.mysteryBoxProductRelForAdminController.query, api.mysteryBoxProductRelForAdminController, {})
)
const {
  loadTableData,
  handleSortChange,
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.mysteryBoxProductRelForAdminController.delete({body: ids}).then((res) => {
  ...
  <div>
    ...
    >
      ...
      <el-table-column label="创建时间" prop="createdTime" show-overflow-tooltip sortable="custom" width="150">
        <template v-slot:default="{ row }: MysteryBoxProductRelScope">
          {{ row.createdTime }}
        </template>
      </el-table-column>
