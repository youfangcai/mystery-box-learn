...
import type {CarriageTemplateDto} from '@/apis/__generated/model/dto'
...
type CarriageTemplateScope = Scope<CarriageTemplateDto['CarriageTemplateRepository/COMPLEX_FETCHER_FOR_ADMIN']>
const carriageTemplateTableHelper = inject(
    'carriageTemplateTableHelper',
    useTableHelper(api.carriageTemplateForAdminController.query, api.carriageTemplateForAdminController, {})
)
const {
  loadTableData,
  handleSortChange,
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.carriageTemplateForAdminController.delete({body: ids}).then((res) => {
...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="模板名称" prop="name" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CarriageTemplateScope">
          {{row.name}}
        </template>
      </el-table-column>
      <el-table-column label="模板描述" prop="description" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CarriageTemplateScope">
          {{row.description}}
        </template>
      </el-table-column>
      <el-table-column label="" prop="valid" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:CarriageTemplateScope">
