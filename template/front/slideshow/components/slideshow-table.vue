...
<template>
  <div>
    ...
    >
      ...
      <el-table-column label="公告图片" prop="picture" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:SlideshowScope">
          <el-avatar :src="row.picture" alt=""></el-avatar>
        </template>
      </el-table-column>
      <el-table-column label="公告内容" prop="content" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:SlideshowScope">
          {{row.content}}
        </template>
      </el-table-column>
      <el-table-column label="跳转ID" prop="navigatorId" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:SlideshowScope">
          {{row.navigatorId}}
        </template>
      </el-table-column>
      <el-table-column label="跳转类型" prop="navigatorType" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:SlideshowScope">
          <dict-column :dict-id="DictConstants.NAVIGATOR_TYPE" :value="row.navigatorType"></dict-column>
        </template>
      </el-table-column>
      <el-table-column label="排序号" prop="sort" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:SlideshowScope">
          {{row.sort}}
        </template>
      </el-table-column>
      <el-table-column label="是否显示" prop="valid" show-overflow-tooltip sortable="custom" width="120">
        <template v-slot:default="{row}:SlideshowScope">
          {{row.valid}}
        </template>
      </el-table-column>
